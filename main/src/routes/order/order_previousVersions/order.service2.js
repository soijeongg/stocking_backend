...
export class OrderService {
  constructor(orderRepository) {
    ...
    this.verifySellingLastOrder = this.verifySellingLastOrder.bind(this);
  }
  ...
  async orderConcludeBuyingProcess(orderData, poppedArray, remainingQuantity) {
    ...
    // 계좌 잔액 증가 - 매도자
    transactionPromises.push(this.orderRepository.increaseUserCurrentMoney(poppedArray.userId, poppedArray.price * remainingQuantity));
  ...
  // 매도 주문 체결
  async orderConcludeSellingProcess(orderData, poppedArray, remainingQuantity) {
    let transactionPromises = [];
  ...
  async concludeInitialBuyingOrder(orderData, initialArray, remainingQuantity) {
    ...
    for (let concludedOrderInfo of initialArray) {
      let userCurrentMoney = await this.orderRepository.getUserCurrentMoney(orderData.userId);
      ...
      transactionPromises.push(this.orderConcludeBuyingProcess(orderData, concludedOrderInfo));
  ...
  async concludeInitialSellingOrder(orderData, initialArray, remainingQuantity) {
    ...
    // initialArray의 각 주문에 대해 매수-매도 양측의 체결기록과 계좌 변동
    for (let concludedOrderInfo of initialArray) {
      transactionPromises.push(this.orderConcludeSellingProcess(orderData, concludedOrderInfo));
      remainingQuantity -= concludedOrderInfo.quantity;
    }
    try {
      await this.orderRepository.transaction(transactionPromises);
  ...
  postMarketPriceOrder = async (userId, receivedOrderData) => {
    ...
    try {
      ...
      if (orderData.type == 'sell') {
        ...
        // ___________1. 매도자의 기록 처리___________(클라이언트)
        // 체결 기록 생성- 매도자
        ...
        // 보유 주식 증가 - 매도자
        const stockIdObject = await this.orderRepository.getUserStockInfo(orderData.userId, orderData.companyId);
        transactionPromises.push(this.orderRepository.decreaseUserStockInfo(stockIdObject.stockId, remainingQuantity));
        // ___________2. 매수자의 기록 처리___________
        // 체결 기록 생성 - 매수자
        ...
        let isStock;
        try {
          isStock = await this.orderRepository.getUserStockInfo(lastExpensiveBuying.userId, orderData.companyId);
        } catch (error) {
          console.log('시장가 매도 부분 isStock에서 에러가 발생했습니다.\n', error.message, '\n', error.stack);
        }
        if (isStock) {
        ...
        try {
          await prisma.$transaction(async (tx) => {
            if (lastCheapestSelling.quantity > remainingQuantity) {
              ...
              await tx.orderRepository.createOrderByOrderData(newBuyingOrder);
            }
            ...
            console.log(remainingQuantity, '여기까지');
            console.log(lastCheapestSelling.price);
            const totalPrice = lastCheapestSelling.price * remainingQuantity;
            console.log(totalPrice, typeof totalPrice);
          });

          // ___________1. 매도자의 기록 처리___________
          // 체결 기록 생성- 매도자
          await this.orderRepository.createConcludedOrder(lastCheapestSelling.userId, orderData.companyId, lastCheapestSelling.type, lastCheapestSelling.price, remainingQuantity);
          // 계좌 잔액 증가 - 매도자
          await this.orderRepository.increaseUserCurrentMoney(lastCheapestSelling.userId, totalPrice);
          // 보유 주식 감소 - 매도자
          try {
            const stockIdObject = await this.orderRepository.getUserStockInfo(lastCheapestSelling.userId, orderData.companyId);
            await this.orderRepository.decreaseUserStockInfo(stockIdObject.stockId, remainingQuantity);
          } catch (error) {
            console.error('order.service 시장가 매수.보유 주식 감소 처리 중 에러가 발생했습니다.');
          }

          // ___________2. 매수자의 기록 처리___________(클라이언트)
          // 체결 기록 생성 - 매수자
          await this.orderRepository.createConcludedOrder(orderData.userId, orderData.companyId, orderData.type, lastCheapestSelling.price, remainingQuantity);

          // 계좌 잔액 감소 - 매수자
          await this.orderRepository.decreaseUserCurrentMoney(orderData.userId, totalPrice);

          // 보유 주식 증가 - 매수자
          // 보유 주식 증가 - 매수자
          try {
            const isStock = await this.orderRepository.getUserStockInfo(orderData.userId, orderData.companyId);
            if (isStock) {
              ...
              await this.orderRepository.increaseUserStockInfo_shareholder(isStock.stockId, newAveragePrice, remainingQuantity);
            } else {
              await this.orderRepository.increaseUserStockInfo_firstBuying(orderData.userId, orderData.companyId, lastCheapestSelling.price, remainingQuantity);
            }
          } catch (error) {
            console.error('order.service 시장가 매수.보유 주식 증가 처리 중 에러 발생');
          }

          // 현재가를 변경
          await this.orderRepository.changeCurrentPrice(orderData.companyId, lastCheapestSelling.price);
          console.log('여기까진 왔음');
          return { message: '정상적으로 시장가 매도 주문이 처리되었습니다.' }; // 생성된 주문 결과 반환
        } catch (error) {
    ...
    try {
      ...
      if (orderData.type == 'sell') {
        ...
        if (orderData.price <= currentPrice) {
          ...
          // last 주문 체결
          try {
            await this.orderConcludeSellingProcess(orderData, lastExpensiveBuying, remainingQuantity);
          } catch (error) {
        ...
        if (orderData.price >= currentPrice) {
          ...
          // last 주문 체결
          try {
            await this.orderConcludeBuyingProcess(orderData, lastCheapestSelling, remainingQuantity);
          } catch (error) {
      ...
      console.log(error.stack);
      return { message: '지정가 주문 생성중에 문제가 생겼습니다. 정확한 문제를 확인하기 위해 고객센터로 연락해주세요. 담당자: 최준혁' };
    }
  };
    ...
    try {
      ...
      if ((orderData.type == 'sell') & (orderData.price <= currentPrice)) {
        ...
        if (orderData.price <= currentPrice) {
          ...
          // 현재 order테이블에 매수 주문이 없을 경우
          if (mostExpensiveBuyings.length == 0) {
            return { message: '정정과정에서 매도가 불가합니다: 현재 해당 주식이 대해 매수 주문이 없습니다.' };
          }
          // 가져온 데이터의 마지막 데이터는 전부 체결이 안될수도 있으므로 따로 관리
          let lastExpensiveBuying = mostExpensiveBuyings.pop();
          ...
          if (mostExpensiveBuyings.length > 0) {
            try {
              ...
              console.log('정정과정에서 order.service 지정가-매도-초기 거래 체결과정에서 에러가 발생했습니다\n', error.status);
            }
          }
          ...
          try {
            ...
            console.log('정정과정에서 order.service 지정가-매도-마지막 체결 이전 verify과정에서 에러가 발생했습니다\n', error.status);
          }
          ...
          try {
            ...
            console.log('정정과정에서 order.service 지정가-매도-마지막 주문 체결과정에서 에러가 발생했습니다\n', error.status);
          }

          return { message: '정정과정에서 정상적으로 지정가 매도 주문이 처리되었습니다.' }; // 생성된 주문 결과 반환
        } else {
          // (orderData.price > currentPrice)
          // 매도 지정가가 현재가보다 높을때 - 지정가 매도 주문을 order테이블에 추가
          await this.orderRepository.createOrderByOrderData(orderData);
        ...
        if (orderData.price <= currentPrice) {
          ...
          if (mostCheapestSellings.length > 0) {
            try {
              ...
              console.log('정정과정에서 order.service 지정가-매수-초기거래 체결과정에서 에러가 발생했습니다\n', error.status);
            }
          }
          ...
          try {
            ...
            console.log('정정과정에서 order.service 지정가-매수-마지막 체결 이전 verify과정에서 에러가 발생했습니다\n', error.status);
          }
          ...
          try {
            ...
            console.log('정정과정에서 order.service 지정가-매수-마지막 주문 체결과정에서 에러가 발생했습니다\n', error.status);
          }

          return { message: '정정과정에서 정상적으로 지정가 매수 주문이 처리되었습니다.' };
        } else {
          // (orderData.price > currentPrice)
          // 매수 지정가 주문을 order테이블에 추가해야함
          await this.orderRepository.createOrderByOrderData(orderData);
          return { message: '정정과정에서 정상적으로 지정가 주문이 생성되었습니다.' };
        }
      } else {
        await this.orderRepository.createOrderByOrderData(orderData);
