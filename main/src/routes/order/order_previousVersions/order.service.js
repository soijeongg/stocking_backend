  ...
  postMarketPriceOrder = async (userId, receivedOrderData) => {
    ...
    try {
      ...
      if (orderData.type == 'sell') {
        ...
        // 계좌 잔액 증가 - 매도자
        transactionPromises.push(this.orderRepository.increaseUserCurrentMoney(lastCheapestSelling.userId, totalPrice));
        // 보유 주식 감소 - 매도자
        transactionPromises.push(
          this.orderRepository
            .getUserStockInfo(lastCheapestSelling.userId, orderData.companyId)
            .then((stockIdObject) => this.orderRepository.decreaseUserStockInfo(stockIdObject.stockId, remainingQuantity))
        );
        ...
        // 계좌 잔액 감소 - 매수자
        transactionPromises.push(this.orderRepository.decreaseUserCurrentMoney(orderData.userId, totalPrice));

        // 보유 주식 증가 - 매수자
        const stockInfoPromise = this.orderRepository.getUserStockInfo(orderData.userId, orderData.companyId).then((isStock) => {
          if (isStock) {
            ...
            return this.orderRepository.increaseUserStockInfo_firstBuying(orderData.userId, orderData.companyId, lastCheapestSelling.price, remainingQuantity);
          }
        });

        // 위에서 생성된 프로미스를 transactionPromises 배열에 추가
        transactionPromises.push(stockInfoPromise);
